...
class EDMSModel(BaseModel):
    ...
    def encode_input_stage1(self, flabel_map=None, real_image=None, ds_image=None, infer=False):
        input_flabel = None
    ...
    def encode_input_stage2(self, flabel_map=None, comp_image=None, ds_image=None, infer=False):
        input_flabel = None
        if not self.opt.no_seg: # seg
          if self.opt.label_nc == 0:
              if flabel_map is not None:
        ...
        return input_flabel, comp_image, ds_image
    ...
    def inference_stage1(self, real, flabel, ds):
        # Encode Inputs
        input_flabel, real_image, ds = self.encode_input_stage1(flabel_map=Variable(flabel), real_image=Variable(real), ds_image=Variable(ds), infer=True)
        ...
        true_residue = upsample(real_image) - fake_image

        return fake_image, comp_image, up_image


    def inference_stage2(self, comp_image, flabel, ds): #self enhancement stage
        # Encode Inputs
        input_flabel, comp_image, ds = self.encode_input_stage2(flabel_map=Variable(flabel), comp_image=Variable(comp_image), ds_image=Variable(ds), infer=True)
        # input to G: DS or compG output
        if self.opt.comp_type=='compG': # use compG
          if len(self.opt.gpu_ids)==0: # cpu
            compG_input = compG_input.cpu()
